...
class DailyBudget extends StatefulWidget {
  ...
  DailyBudget({Key key, this.project}) : super(key: key);

  @override
  _DailyBudget createState() => _DailyBudget();
}

class _DailyBudget extends State<DailyBudget>
    with SingleTickerProviderStateMixin {
  final Project project;

  _DailyBudget({this.project});
  ...
  Widget build(BuildContext context) {
    ...
    callSheetControllers = [];
    rateControllers = [];
    categories = dailyBudget.keys.toList();
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            children: [
              ...
              Column(
                children: List<Widget>.generate(dailyBudget.length, (i) {
                  subCategories = dailyBudget[categories[i]].keys.toList();
                  return Column(
                    children: [
                      Divider(
                        thickness: 1,
                            ...
                            InkWell(
                              onTap: () {
                                createAlertDialog(context, "SubCategory")
                                    .then((v) {
                                  if (v == null) {
                                  } else if (dailyBudget[categories[i]]
                                          ['${v}'] ==
                                      null) {
                                    dailyBudget[categories[i]]['$v'] = {
                                      'contact': '',
                                      'quantity': 0,
                                      'rate': 0,
                                      'subtotal': 0,
                      ...
                      Padding(
                        ...
                        child: Column(
                          ...
                          var subcategory =
                              dailyBudget[categories[i]][subCategories[j]];
                                    ...
                                    Row(
                                      children: [
                                        Transform.scale(
                                          scale: 1.1,
                                          child: Checkbox(
                                              value: dailyBudget[categories[i]]
                                                  [subCategories[j]]["use"],
                                              activeColor: color,
                                              onChanged: (value) {
                                                setState(() {
                                                  dailyBudget[categories[i]]
                                                          [subCategories[j]]
                                                      ["use"] = value;
                                                });
                                              }),
                                      ...
                                      children: [
                                        ...
                                        Flexible(
                                          child: Padding(
                                            padding: const EdgeInsets.all(8.0),
                                            child: Text(
                                              "Subtotal: ${dailyBudget[categories[i]][subCategories[j]]["subtotal"]}",
                                              style: subheading,
                                            ),
                                          ),
                                        ),
